...
import {
  ...
  async function inputOpenAiKey(context: ExtensionContext, invalidKey = false) {
    ...
    if (await validApiKey(result)) {
      ...
      let OPENAI_API_KEY = context.globalState.get("OpenAI_API_KEY");
  ...
  export function activate(context: ExtensionContext) {
    context.subscriptions.push(
      commands.registerCommand("tutor.init", async () => {
        let OPENAI_API_KEY: any = context.globalState.get("OpenAI_API_KEY");
...
let language, selectedFunctionName, sourceCodeOfSelectedFunction, selectedText, textAtLineOfCursor;
