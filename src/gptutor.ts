...
import { processCodeToRemoveTypeAnnotation } from "./suiGPT/processCodeToRemoveTypeAnnotation";
...
export class GPTutor implements vscode.WebviewViewProvider {
  ...
  async runChatGPT(
    ...
    try {
      ...
      if (true || error?.message.includes("maximum context length")) {
        try {
          if (
            ...
            vscode.window.showInformationMessage(
              `Using "gpt-4" since "gpt-4-1106-preview" reach it's request limit`
            );
            await this.runChatGPT(
              prompt,
              (model = "gpt-4"),
  ...
  async insertToActiveEditor(
    ...
    await editor.edit((editBuilder: any) => {
      ...
      let text_message = total_text_so_far.split("```")[0];
      total_text_so_far = total_text_so_far.split("```").slice(1).join("```");

      total_text_so_far = "// Generated by SuiGPT" + total_text_so_far;
      total_text_so_far = total_text_so_far.replace("```", "");
      total_text_so_far = processCodeToRemoveTypeAnnotation(total_text_so_far);
  ...
  public async active(mode: string, model: string = "gpt-4-1106-preview") {
    this.currentPrompt = await getCurrentPromptV2(
      this.context,
      this.cursorContext
    );
